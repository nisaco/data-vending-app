...
app.post('/paystack/verify', async (req, res) => {
    ...
    try {
        ...
        if (status && data.status === 'success') {
            ...
            // 1. Save order to database to prevent re-fulfilling
            const checkStmt = db.prepare('SELECT status FROM orders WHERE reference = ?');
            checkStmt.get(reference, (err, row) => {
                if (err) {
                ...
                if (row && row.status === 'success') {
                    return res.json({ status: 'success', message: 'Order already fulfilled.' });
                }
                
                // Insert new order or update pending one
                ...
                stmt.run(
                    ...
                    orderDetails.data_plan, 
                    amountInGHS,
                    (err) => {
                        if (err) {
                           return console.error("Error saving order:", err.message);
                        }
                         // 2. TODO: Call your Data Vending API here to deliver the data
...
app.post('/paystack/webhook', (req, res) => {
    ...
    if (event.event === 'charge.success') {
        ...
        // Logic is similar to verification: save to DB and fulfill order
        // This is a great fallback if the user closes their browser too quickly
         const stmt = db.prepare(`
            INSERT INTO orders (reference, email, phone_number, network, data_plan, amount, status)
            VALUES (?, ?, ?, ?, ?, ?, 'success')
            ON CONFLICT(reference) DO UPDATE SET status = 'success'
        ...
        stmt.run(
            ...
            (err) => {
                if (err) {
                    return console.error("Webhook DB Error:", err.message);
                }
                // TODO: Call your Data Vending API here as well.
                // Ensure you have logic to avoid double-fulfillment!
...
// A simple "database" of your data plans. In a real app, this might come from a database.
const allPlans = {
    "MTN": [
...
// Updated endpoint to send specific plans based on the selected network
app.get('/api/data-plans', (req, res) => {
    const network = req.query.network; // Get network from URL (e.g., 'MTN')
    
    if (network && allPlans[network]) {
        // If the network exists in our list, send its specific plans
