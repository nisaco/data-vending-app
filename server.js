...
app.post('/api/topup', isDbReady, isAuthenticated, async (req, res) => {
    ...
    try {
        ...
        await Order.create({
            ...
            network: 'WALLET' // 🛑 CRITICAL FIX: Ensure network field is set for filtering
        });
...
app.get('/api/get-all-orders', async (req, res) => {
    ...
    try {
        // 🛑 FIX: Use $or to fetch data purchases AND top-up records
        const orders = await Order.find({
            $or: [
                { network: { $exists: true, $ne: 'WALLET' } }, // Get all data purchases
                { network: 'WALLET' } // Get all wallet transactions
            ]
        })
        .sort({ createdAt: -1 })
        ...
        const formattedOrders = orders.map(order => ({
            ...
            network: order.network || 'WALLET', // Ensure network is labeled
